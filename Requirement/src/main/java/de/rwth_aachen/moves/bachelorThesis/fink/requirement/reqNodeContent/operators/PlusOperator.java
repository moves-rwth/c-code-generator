...
public class PlusOperator extends OperatorTreeNode {
	public static final int ARGUMENT_COUNT = 2;

	public PlusOperator(ImmutableList<FormulaTreeNode> children, boolean isInOutput) {
		super("+", children, isInOutput);
	}

	protected PlusOperator(PlusOperator other, IVariableReplacer variableReplacer) {
		super(other, variableReplacer);
	}

	protected PlusOperator(PlusOperator other, ImmutableList<FormulaTreeNode> newChildren) {
	...
	public static SimpleChildExpressionConditioner makeChildExpressionConditioner(int childId, SimpleChildExpressionConditioner parentConditioner, List<SimpleResultingExpressionConditioner> previousChildResultConditioners, IDataTypeContext dataTypeContext) throws UnsatisfiableConstraintsException {
		...
		SimpleChildExpressionConditionerBuilder result = parentConditioner.mathExpr(PlusOperator.class, parentConditioner, previousChildResultConditioners);
		return result.setCanBeASimpleValue(true).setParentOperator(Operators.PLUS).build();
	}
	...
	public SimpleResultingExpressionConditioner getResultingExpressionConditioner(IProgramContext programContext) throws UnsatisfiableConstraintsException {
		...
		return SimpleResultingExpressionConditioner.combineMathExpression(PlusOperator.class, childResultConditionerA, childResultConditionerB);
	}
	...
	public FormulaTreeNode replaceChildren(ImmutableList<FormulaTreeNode> children) {
		return new PlusOperator(this, children);
	}
	...
	public CodeTreeNode copyTree(IVariableReplacer variableReplacer) {
		return new PlusOperator(this, variableReplacer);
	}
}
