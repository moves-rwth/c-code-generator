...
public class BitShiftRightOperator extends OperatorTreeNode {
	public static final int ARGUMENT_COUNT = 2;

	public BitShiftRightOperator(ImmutableList<FormulaTreeNode> children, boolean isInOutput) {
		super(">>", children, isInOutput);
	}

	protected BitShiftRightOperator(BitShiftRightOperator other, IVariableReplacer variableReplacer) {
		super(other, variableReplacer);
	}

	protected BitShiftRightOperator(BitShiftRightOperator other, ImmutableList<FormulaTreeNode> newChildren) {
	...
	public static SimpleChildExpressionConditioner makeChildExpressionConditioner(int childId, SimpleChildExpressionConditioner parentConditioner, List<SimpleResultingExpressionConditioner> previousChildResultConditioners, IDataTypeContext dataTypeContext) throws UnsatisfiableConstraintsException {
		...
		if (childId == 0) {
			...
			// b
			result = parentConditioner.bitShift(true, parentConditioner, previousChildResultConditioners, dataTypeContext);
		}
		return result.setCanBeASimpleValue(true).setParentOperator(Operators.BIT_SHIFT_RIGHT).build();
	}
	...
	public FormulaTreeNode replaceChildren(ImmutableList<FormulaTreeNode> newChildren) {
		return new BitShiftRightOperator(this, newChildren);
	}
	...
	public CodeTreeNode copyTree(IVariableReplacer variableReplacer) {
		return new BitShiftRightOperator(this, variableReplacer);
	}
}
