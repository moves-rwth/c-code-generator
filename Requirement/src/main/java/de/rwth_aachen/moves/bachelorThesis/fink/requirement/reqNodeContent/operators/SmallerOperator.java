...
public class SmallerOperator extends ComparisonOperator {
	public static final int ARGUMENT_COUNT = 2;

	public SmallerOperator(ImmutableList<FormulaTreeNode> children, boolean isInOutput) {
		super("<", children, isInOutput);
	}

	protected SmallerOperator(SmallerOperator other, IVariableReplacer variableReplacer) {
		super(other, variableReplacer);
	}

	protected SmallerOperator(SmallerOperator other, ImmutableList<FormulaTreeNode> newChildren) {
	...
	public static SimpleChildExpressionConditioner makeChildExpressionConditioner(int childId, SimpleChildExpressionConditioner parentConditioner, List<SimpleResultingExpressionConditioner> previousChildResultConditioners, IDataTypeContext dataTypeContext) throws UnsatisfiableConstraintsException {
		...
		SimpleChildExpressionConditionerBuilder result = parentConditioner.equalsExpr(SmallerOperator.class, parentConditioner, previousChildResultConditioners, dataTypeContext);
		return result.setCanBeASimpleValue(true).setParentOperator(Operators.SMALLER).build();
	}
	...
	public FormulaTreeNode replaceChildren(ImmutableList<FormulaTreeNode> children) {
		return new SmallerOperator(this, children);
	}
	...
	public CodeTreeNode copyTree(IVariableReplacer variableReplacer) {
		return new SmallerOperator(this, variableReplacer);
	}
}
