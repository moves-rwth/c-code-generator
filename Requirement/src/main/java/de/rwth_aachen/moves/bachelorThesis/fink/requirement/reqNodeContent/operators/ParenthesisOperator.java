...
public class ParenthesisOperator extends OperatorTreeNode {
	public static final int ARGUMENT_COUNT = 1;

	public ParenthesisOperator(ImmutableList<FormulaTreeNode> children, boolean isInOutput) {
		super("(param)", children, isInOutput);
	}

	protected ParenthesisOperator(ParenthesisOperator other, IVariableReplacer variableReplacer) {
		super(other, variableReplacer);
	}

	protected ParenthesisOperator(ParenthesisOperator other, ImmutableList<FormulaTreeNode> newChildren) {
	...
	public StringArray toCode(CodeType codeType, Set<IVariableWithAccessor> usedVariables, IProgramContext context) {
		if (codeType == CodeType.EXECUTION) {
			return getExecutionChildCode(0, usedVariables, context);
		}

		StringArray result = new StringArray();
		result.add("(");
		result.addIndented(getChildCode(0, usedVariables, context));
		result.add(")");
		return result;
	}
	...
	public EnumSet<Operators> getPossibleReplacementOperators() {
		return EnumSet.of(Operators.PARENTHESIS, Operators.EMPTY);
	}
	...
	public static SimpleChildExpressionConditioner makeChildExpressionConditioner(int childId, SimpleChildExpressionConditioner parentConditioner, List<SimpleResultingExpressionConditioner> previousChildResultConditioners, IDataTypeContext dataTypeContext) throws UnsatisfiableConstraintsException {
		...
		return new SimpleChildExpressionConditionerBuilder(parentConditioner).setParentOperator(Operators.PARENTHESIS).build();
	}
	...
	public FormulaTreeNode replaceChildren(ImmutableList<FormulaTreeNode> children) {
		return new ParenthesisOperator(this, children);
	}
	...
	public CodeTreeNode copyTree(IVariableReplacer variableReplacer) {
		return new ParenthesisOperator(this, variableReplacer);
	}
}
