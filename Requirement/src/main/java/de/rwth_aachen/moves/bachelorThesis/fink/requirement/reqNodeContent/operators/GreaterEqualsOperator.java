...
public class GreaterEqualsOperator extends ComparisonOperator {
	public static final int ARGUMENT_COUNT = 2;

	public GreaterEqualsOperator(ImmutableList<FormulaTreeNode> children, boolean isInOutput) {
		super(">=", children, isInOutput);
	}

	protected GreaterEqualsOperator(GreaterEqualsOperator other, IVariableReplacer variableReplacer) {
		super(other, variableReplacer);
	}

	protected GreaterEqualsOperator(GreaterEqualsOperator other, ImmutableList<FormulaTreeNode> newChildren) {
	...
	public static SimpleChildExpressionConditioner makeChildExpressionConditioner(int childId, SimpleChildExpressionConditioner parentConditioner, List<SimpleResultingExpressionConditioner> previousChildResultConditioners, IDataTypeContext dataTypeContext) throws UnsatisfiableConstraintsException {
		...
		SimpleChildExpressionConditionerBuilder result = parentConditioner.equalsExpr(GreaterEqualsOperator.class, parentConditioner, previousChildResultConditioners, dataTypeContext);
		return result.setCanBeASimpleValue(true).setParentOperator(Operators.GREATER_EQUALS).build();
	}
	...
	public FormulaTreeNode replaceChildren(ImmutableList<FormulaTreeNode> children) {
		return new GreaterEqualsOperator(this, children);
	}
	...
	public CodeTreeNode copyTree(IVariableReplacer variableReplacer) {
		return new GreaterEqualsOperator(this, variableReplacer);
	}
}
