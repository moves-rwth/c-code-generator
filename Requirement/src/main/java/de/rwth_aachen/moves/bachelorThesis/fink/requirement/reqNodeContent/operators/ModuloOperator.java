...
public class ModuloOperator extends OperatorTreeNode {
	public static final int ARGUMENT_COUNT = 2;

	public ModuloOperator(ImmutableList<FormulaTreeNode> children, boolean isInOutput) {
		super("%", children, isInOutput);
	}

	protected ModuloOperator(ModuloOperator other, IVariableReplacer variableReplacer) {
		super(other, variableReplacer);
	}

	protected ModuloOperator(ModuloOperator other, ImmutableList<FormulaTreeNode> newChildren) {
	...
	public static SimpleChildExpressionConditioner makeChildExpressionConditioner(int childId, SimpleChildExpressionConditioner parentConditioner, List<SimpleResultingExpressionConditioner> previousChildResultConditioners, IDataTypeContext dataTypeContext) throws UnsatisfiableConstraintsException {
		...
		SimpleChildExpressionConditionerBuilder result = parentConditioner.mathExpr(ModuloOperator.class, parentConditioner, previousChildResultConditioners);
		return result.setCanBeASimpleValue(true).setParentOperator(Operators.MODULO).build();
	}
	...
	public SimpleResultingExpressionConditioner getResultingExpressionConditioner(IProgramContext programContext) throws UnsatisfiableConstraintsException {
		...
		return SimpleResultingExpressionConditioner.combineMathExpression(ModuloOperator.class, childResultConditionerA, childResultConditionerB);
	}
	...
	public FormulaTreeNode replaceChildren(ImmutableList<FormulaTreeNode> children) {
		return new ModuloOperator(this, children);
	}

	@Override
	public CodeTreeNode copyTree(IVariableReplacer variableReplacer) {
		return new ModuloOperator(this, variableReplacer);
	}
}
