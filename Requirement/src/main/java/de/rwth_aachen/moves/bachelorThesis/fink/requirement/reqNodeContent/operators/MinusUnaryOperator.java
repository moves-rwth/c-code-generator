...
public class MinusUnaryOperator extends OperatorTreeNode {
	public static final int ARGUMENT_COUNT = 1;

	public MinusUnaryOperator(ImmutableList<FormulaTreeNode> children, boolean isInOutput) {
		super("minusUnary", children, isInOutput);
	}

	protected MinusUnaryOperator(MinusUnaryOperator other, IVariableReplacer variableReplacer) {
		super(other, variableReplacer);
	}

	protected MinusUnaryOperator(MinusUnaryOperator other, ImmutableList<FormulaTreeNode> newChildren) {
	...
	public static SimpleChildExpressionConditioner makeChildExpressionConditioner(int childId, SimpleChildExpressionConditioner parentConditioner, List<SimpleResultingExpressionConditioner> previousChildResultConditioners, IDataTypeContext dataTypeContext) throws UnsatisfiableConstraintsException {
		...
		SimpleChildExpressionConditionerBuilder result = parentConditioner.mathExpr(MinusUnaryOperator.class, parentConditioner, previousChildResultConditioners);
		return result.setCanBeASimpleValue(true).setParentOperator(Operators.MINUS_UNARY).build();
	}
	...
	public FormulaTreeNode replaceChildren(ImmutableList<FormulaTreeNode> children) {
		return new MinusUnaryOperator(this, children);
	}
	...
	public CodeTreeNode copyTree(IVariableReplacer variableReplacer) {
		return new MinusUnaryOperator(this, variableReplacer);
	}
}
