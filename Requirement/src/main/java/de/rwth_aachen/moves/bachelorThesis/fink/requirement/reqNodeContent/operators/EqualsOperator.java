...
public class EqualsOperator extends ComparisonOperator {
	...
	public EqualsOperator(ImmutableList<FormulaTreeNode> children, boolean isInOutput) {
		super("equals", children, isInOutput);
	}

	protected EqualsOperator(EqualsOperator other, IVariableReplacer variableReplacer) {
		super(other, variableReplacer);
	}

	protected EqualsOperator(EqualsOperator other, ImmutableList<FormulaTreeNode> newChildren) {
	...
	public static SimpleChildExpressionConditioner makeChildExpressionConditioner(int childId, SimpleChildExpressionConditioner parentConditioner, List<SimpleResultingExpressionConditioner> previousChildResultConditioners, IDataTypeContext dataTypeContext) throws UnsatisfiableConstraintsException {
		...
		SimpleChildExpressionConditionerBuilder result = parentConditioner.equalsExpr(EqualsOperator.class, parentConditioner, previousChildResultConditioners, dataTypeContext);
		return result.setCanBeASimpleValue(true).setParentOperator(Operators.EQUALS).build();
	}
	...
	public FormulaTreeNode replaceChildren(ImmutableList<FormulaTreeNode> children) {
		return new EqualsOperator(this, children);
	}
	...
	public CodeTreeNode copyTree(IVariableReplacer variableReplacer) {
		return new EqualsOperator(this, variableReplacer);
	}
}
