...
public class PrefixIncrementOperator extends OperatorTreeNode {
	public static final int ARGUMENT_COUNT = 1;

	public PrefixIncrementOperator(ImmutableList<FormulaTreeNode> children, boolean isInOutput) {
		super("prefixIncrement", children, isInOutput);
	}

	protected PrefixIncrementOperator(PrefixIncrementOperator other, IVariableReplacer variableReplacer) {
		super(other, variableReplacer);
	}

	protected PrefixIncrementOperator(PrefixIncrementOperator other, ImmutableList<FormulaTreeNode> newChildren) {
	...
	public static SimpleChildExpressionConditioner makeChildExpressionConditioner(int childId, SimpleChildExpressionConditioner parentConditioner, List<SimpleResultingExpressionConditioner> previousChildResultConditioners, IDataTypeContext dataTypeContext) throws UnsatisfiableConstraintsException {
		...
		SimpleChildExpressionConditionerBuilder result = parentConditioner.mathExpr(PrefixIncrementOperator.class, parentConditioner, previousChildResultConditioners);
	...
	public FormulaTreeNode replaceChildren(ImmutableList<FormulaTreeNode> children) {
		return new PrefixIncrementOperator(this, children);
	}

	@Override
	public DataType getDynamicReturnType(IProgramContext programContext) {
		// NOTE(Felix):
		// i8 variableWithTruncation = 127;
		// i32 testVariable = ++variableWithTruncation; -> testVariable = -128
	...
	public CodeTreeNode copyTree(IVariableReplacer variableReplacer) {
		return new PrefixIncrementOperator(this, variableReplacer);
	}
}
