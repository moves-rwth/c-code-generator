...
public class OrOperator extends OperatorTreeNode {
	public static final int ARGUMENT_COUNT = 2;

	public OrOperator(ImmutableList<FormulaTreeNode> children, boolean isInOutput) {
		super("||", children, isInOutput);
	}

	protected OrOperator(OrOperator other, IVariableReplacer variableReplacer) {
		super(other, variableReplacer);
	}

	protected OrOperator(OrOperator other, ImmutableList<FormulaTreeNode> newChildren) {
	...
	public static SimpleChildExpressionConditioner makeChildExpressionConditioner(int childId, SimpleChildExpressionConditioner parentConditioner, List<SimpleResultingExpressionConditioner> previousChildResultConditioners, IDataTypeContext dataTypeContext) throws UnsatisfiableConstraintsException {
		...
		if (!booleanRange.canBeFalse()) {
			result.setRange(new SimpleBooleanRange(true, previousChildResultConditioners.isEmpty()));
		} else {
			result.setRange(new SimpleBooleanRange(booleanRange.canBeTrue(), true));
		}

		return result.setCanBeASimpleValue(false).setParentOperator(Operators.OR).build();
	}
	...
	public FormulaTreeNode replaceChildren(ImmutableList<FormulaTreeNode> children) {
		return new OrOperator(this, children);
	}
	...
	public CodeTreeNode copyTree(IVariableReplacer variableReplacer) {
		return new OrOperator(this, variableReplacer);
	}
}
