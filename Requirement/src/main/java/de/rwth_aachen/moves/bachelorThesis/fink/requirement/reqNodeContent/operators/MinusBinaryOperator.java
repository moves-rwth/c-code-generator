...
public class MinusBinaryOperator extends OperatorTreeNode {
	public static final int ARGUMENT_COUNT = 2;

	public MinusBinaryOperator(ImmutableList<FormulaTreeNode> children, boolean isInOutput) {
		super("minusBinary", children, isInOutput);
	}

	protected MinusBinaryOperator(MinusBinaryOperator other, IVariableReplacer variableReplacer) {
		super(other, variableReplacer);
	}

	protected MinusBinaryOperator(MinusBinaryOperator other, ImmutableList<FormulaTreeNode> newChildren) {
	...
	public static SimpleChildExpressionConditioner makeChildExpressionConditioner(int childId, SimpleChildExpressionConditioner parentConditioner, List<SimpleResultingExpressionConditioner> previousChildResultConditioners, IDataTypeContext dataTypeContext) throws UnsatisfiableConstraintsException {
		...
		SimpleChildExpressionConditionerBuilder result = parentConditioner.mathExpr(MinusBinaryOperator.class, parentConditioner, previousChildResultConditioners);
		return result.setCanBeASimpleValue(true).setParentOperator(Operators.MINUS_BINARY).build();
	}
	...
	public SimpleResultingExpressionConditioner getResultingExpressionConditioner(IProgramContext programContext) throws UnsatisfiableConstraintsException {
		...
		return SimpleResultingExpressionConditioner.combineMathExpression(MinusBinaryOperator.class, childResultConditionerA, childResultConditionerB);
	}
	...
	public FormulaTreeNode replaceChildren(ImmutableList<FormulaTreeNode> children) {
		return new MinusBinaryOperator(this, children);
	}
	...
	public CodeTreeNode copyTree(IVariableReplacer variableReplacer) {
		return new MinusBinaryOperator(this, variableReplacer);
	}
}
