...
public class NotEqualsOperator extends ComparisonOperator {
	...
	public NotEqualsOperator(ImmutableList<FormulaTreeNode> children, boolean isInOutput) {
		super("!=", children, isInOutput);
	}

	protected NotEqualsOperator(NotEqualsOperator other, IVariableReplacer variableReplacer) {
		super(other, variableReplacer);
	}

	protected NotEqualsOperator(NotEqualsOperator other, ImmutableList<FormulaTreeNode> newChildren) {
	...
	public static SimpleChildExpressionConditioner makeChildExpressionConditioner(int childId, SimpleChildExpressionConditioner parentConditioner, List<SimpleResultingExpressionConditioner> previousChildResultConditioners, IDataTypeContext dataTypeContext) throws UnsatisfiableConstraintsException {
		...
		SimpleChildExpressionConditionerBuilder result = parentConditioner.equalsExpr(NotEqualsOperator.class, parentConditioner, previousChildResultConditioners, dataTypeContext);
		return result.setCanBeASimpleValue(true).setParentOperator(Operators.NOT_EQUALS).build();
	}
	...
	public FormulaTreeNode replaceChildren(ImmutableList<FormulaTreeNode> children) {
		return new NotEqualsOperator(this, children);
	}
	...
	public CodeTreeNode copyTree(IVariableReplacer variableReplacer) {
		return new NotEqualsOperator(this, variableReplacer);
	}
}
