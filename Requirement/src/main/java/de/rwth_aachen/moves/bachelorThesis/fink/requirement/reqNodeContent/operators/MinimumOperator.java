...
public class MinimumOperator extends OperatorTreeNode {
	public static final int ARGUMENT_COUNT = 2;

	public MinimumOperator(ImmutableList<FormulaTreeNode> children, boolean isInOutput) {
		super("min", children, isInOutput);
	}

	protected MinimumOperator(MinimumOperator other, IVariableReplacer variableReplacer) {
		super(other, variableReplacer);
	}

	protected MinimumOperator(MinimumOperator other, ImmutableList<FormulaTreeNode> newChildren) {
	...
	public StringArray toCode(CodeType codeType, Set<IVariableWithAccessor> usedVariables, IProgramContext context) {
		return debugFormat("min", debugFormat(getChildCode(0, usedVariables, context), ",", getChildCode(1, usedVariables, context)));
	}
	...
	public static SimpleChildExpressionConditioner makeChildExpressionConditioner(int childId, SimpleChildExpressionConditioner parentConditioner, List<SimpleResultingExpressionConditioner> previousChildResultConditioners, IDataTypeContext dataTypeContext) throws UnsatisfiableConstraintsException {
		...
		SimpleChildExpressionConditionerBuilder result = parentConditioner.mathExpr(MinimumOperator.class, parentConditioner, previousChildResultConditioners);
		return result.setCanBeASimpleValue(true).setParentOperator(Operators.MINIMUM).build();
	}
	...
	public SimpleResultingExpressionConditioner getResultingExpressionConditioner(IProgramContext programContext) throws UnsatisfiableConstraintsException {
		...
		return SimpleResultingExpressionConditioner.combineMathExpression(MinimumOperator.class, childResultConditionerA, childResultConditionerB);
	}
	...
	public FormulaTreeNode replaceChildren(ImmutableList<FormulaTreeNode> children) {
		return new MinimumOperator(this, children);
	}
	...
	public CodeTreeNode copyTree(IVariableReplacer variableReplacer) {
		return new MinimumOperator(this, variableReplacer);
	}
}
